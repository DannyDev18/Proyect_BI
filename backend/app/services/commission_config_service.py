...
class CommissionConfigService:
    def __init__(self, commission_config_repo: CommissionConfigRepository, goal_repo: GoalRepository):
    ...
    def replace_factores_credito(self, factores: list[dict]) -> list[dict]:
    ...
    def get_config_vendedores(self) -> list[dict]:
        return [
            ...
            for v in self.commission_config_repo.get_all_config_vendedores()
        ]

    def upsert_config_vendedor(self, vendedor_origen: str, tipo: str, factor_tipo: float, fecha_ingreso) -> dict:
