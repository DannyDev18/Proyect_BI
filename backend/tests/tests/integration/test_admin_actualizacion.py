...
# ── Fase 5 (docs/features/plan_correcciones_integrales_sistema.md): paginación real,
# borrado permanente y métricas del dashboard de Admin ─────────────────────────
def test_list_users_devuelve_pagina_tipada(client, auth_headers):
    r = client.get("/api/v1/users/?page=1&page_size=5", headers=auth_headers("administrador"))
    assert r.status_code == 200
    body = r.json()
    assert set(body.keys()) >= {"items", "total", "page", "page_size", "total_pages"}
...
def test_delete_permanente_propia_cuenta_es_rechazado(client, auth_headers):
    headers = auth_headers("administrador")
    me = client.get("/api/v1/users/me", headers=headers).json()
    r = client.delete(f"/api/v1/users/{me['id']}/permanente", headers=headers)
    assert r.status_code == 400


def test_delete_permanente_elimina_usuario_real(client, auth_headers):
    email = f"borrar_permanente_{uuid.uuid4().hex[:8]}@empresa.com"
    creado = client.post(
        "/api/v1/users/",
        json={"nombre": "Borrar Permanente", "email": email, "password": "Password123!", "rol_id": 1},
        headers=auth_headers("administrador"),
    )
    assert creado.status_code == 201
    user_id = creado.json()["id"]

    r = client.delete(f"/api/v1/users/{user_id}/permanente", headers=auth_headers("administrador"))
    assert r.status_code == 200

    verificacion = client.get(f"/api/v1/users/{user_id}", headers=auth_headers("administrador"))
    assert verificacion.status_code == 404


def test_admin_resumen_devuelve_metricas_reales(client, auth_headers):
    r = client.get("/api/v1/analytics/admin/resumen", headers=auth_headers("administrador"))
    assert r.status_code == 200
    body = r.json()
    assert set(body.keys()) == {
        "usuarios_activos", "usuarios_inactivos", "total_vendedores_activos", "total_almacenes",
    }
    assert body["usuarios_activos"] >= 1  # el propio admin autenticado
    assert all(isinstance(v, int) for v in body.values())


def test_admin_resumen_rechaza_roles_no_admin(client, auth_headers):
    r = client.get("/api/v1/analytics/admin/resumen", headers=auth_headers("ventas"))
    assert r.status_code == 403
