...
def test_predict_demand_selecciona_solo_las_columnas_del_modelo(fake_model_loader):
    X = pd.DataFrame({"f1": [1.0], "f2": [2.0], "columna_extra_no_usada": [99.0]})
    result = inference.predict_demand(fake_model_loader, X)
