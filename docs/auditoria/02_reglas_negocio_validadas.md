...
## 17. Módulo Venta Cruzada (Cross-Selling): formato de sugerencia y telemetría (RN-CS1, RN-CS2)
...
- **RN-CS1 (formato de sugerencia):** dado un conjunto de `codart` en la canasta simulada (y opcionalmente un `cliente_id`), se devuelven hasta `CROSS_SELL_TOP_N` (default 5) sugerencias con `codart`, `nombre`, `precio` (`dim_producto.precio_oficial` vigente), `categoria` (`clase`), `score` y `motivo`. Se excluyen productos ya en la canasta y, si hay `cliente_id`, los ya comprados por ese cliente. Solo entran reglas con `lift ≥ CROSS_SELL_MIN_LIFT`; si ningún producto de la canasta tiene una regla que cumpla el umbral, se usa el fallback por popularidad de categoría (producto más vendido de la misma `clase` que no esté ya en la canasta). El factor de margen (`CROSS_SELL_PESO_MARGEN`) solo se aplica cuando `dim_producto.costo_promedio` es no nulo y > 0 (≈92% del catálogo vigente, auditoría 25 §1); si no, se ordena solo por lift/score.
