...
- **Estado:** ✅ **Fases 1, 2, 3 y 4 implementadas (2026-07-09).** Fase 1 (infraestructura de contratos), Fase 2 (adaptación de extracción/repositorios al nuevo EDW, ampliada a `analytics_repository.py`/`goal_repository.py` — ver [docs/auditoria/13_impacto_dim_estado_documento.md](13_impacto_dim_estado_documento.md)), Fase 3 (reconstrucción de los 7 modelos, todos los contratos en `ml/contracts/models/*.json` en `status: "active"`, suite `ml/tests/test_model_contract.py` 15/15 sin xfail) y Fase 4 (integración de serving: `backend/app/ml/model_loader.py` apunta a los 7 `.pkl` reconstruidos, features seleccionadas vía el sidecar `.meta.json` en vez de `feature_names_in_`, H-01/H-02/H-03/H-04/H-06/H-09/H-10/H-12 cerrados también del lado del backend) están completas. Se detectó y cerró además un H-20 real (no solo teórico): `ml/requirements.txt` sin techo de versión resolvía pandas 3.0.3 mientras el backend fija `<3.0.0`, y `recommendation.pkl` (el único artefacto que serializa un DataFrame crudo) no cargaba en el backend por el cambio de dtype de string entre versiones -- se alinearon los techos de versión de las 5 librerías compartidas y se reentrenaron los 7 modelos bajo esas versiones. Smoke test end-to-end de los 6 casos de uso ML ejecutado dentro del contenedor backend real contra el EDW: los 6 responden con valores reales, sin degradación silenciosa a `0.0`/`"Error"`. Detalle de uso en [docs/ml_contracts.md](../ml_contracts.md).
