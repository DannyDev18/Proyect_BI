  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <ChartCard
        ...
        badge={{ label: 'IA', variant: 'ml' }}
        loading={loading}
        error={error ?? undefined}
        empty={data.vendedores_en_riesgo.length === 0}
        emptyTitle="Sin vendedores en riesgo"
        emptyDescription="Ningún vendedor por debajo del ritmo esperado."
      >
      ...
      <ChartCard
        title="Alta probabilidad de superar la meta"
        badge={{ label: 'IA', variant: 'ml' }}
        loading={loading}
        error={error ?? undefined}
        empty={data.vendedores_alta_probabilidad.length === 0}
        emptyTitle="Sin destacados"
        emptyDescription="Ningún vendedor destaca sobre el ritmo esperado todavía."
      >
        <ul className="space-y-2 overflow-auto max-h-[260px]">
      ...
      <ChartCard
        title="Recomendaciones comerciales por categoría"
        badge={{ label: 'Reglas Asociación', variant: 'ml' }}
        loading={loading}
        error={error ?? undefined}
        empty={data.recomendaciones_por_categoria.length === 0}
        emptyTitle="Sin recomendaciones"
        emptyDescription="Sin reglas de asociación disponibles."
      >
        <ul className="space-y-2 overflow-auto max-h-[260px]">
